...
def view_attendance():
    ...
    # Get student information to filter attendance records
    ...
    attendance_records = db.session.query(Attendance, Course).join(Course).filter(Attendance.student_id == student.student_id).all()

    if not attendance_records:
        flash("No attendance records found.", "info")

    return render_template('student/view_attendance.html', attendance=attendance_records)
